...
#define STRESS_FUNCTION s21_ceil
#define HEALTHY_FUNCTION ceil
#define STRESS_FUNCTION_Str "s21_ceil(x)"
#define HEALTHY_FUNCTION_Str "ceil(x)"
#define OUTPUT_DIGITS_LENGTH \
  35  // количество символов после запятой, выводимых в терминале

#define FUNCTION_FROM_COMPARE_C compare_double  // определяем какую
// функцию будем использовать

int compare_double(
    double x, double healthy,
    long double stress) {  // возвращает 1 если знач одинаковое, 0 если разные
  printf(HEALTHY_FUNCTION_Str " | x = %.30lf\n", x);
...
Suite *add_suite_HEALTHY_FUNCTION(void) {
  Suite *suit = suite_create("LOG_TEST");
  TCase *tc_core = tcase_create("LOG_TEST");
