...
#define STRESS_FUNCTION s21_abs
#define HEALTHY_FUNCTION abs
#define STRESS_FUNCTION_Str "s21_abs(x)"
#define HEALTHY_FUNCTION_Str "abs(x)"
#define OUTPUT_DIGITS_LENGTH \
  1  // количество символов после запятой, выводимых в терминале

#define FUNCTION_FROM_COMPARE_C \
  compare_int  // определяем какую функцию будем использовать

int compare_int(
    int x, int healthy,
    int stress) {  // возвращает 1 если знач одинаковое, 0 если разные
  int result = 0;

  printf(HEALTHY_FUNCTION_Str " | x = %d\n", x);
  printf("%-*s: %-*.*d\n", 10, HEALTHY_FUNCTION_Str, 25, OUTPUT_DIGITS_LENGTH,
         HEALTHY_FUNCTION(x));
  printf("%-*s: %-*.*d\n\n", 10, STRESS_FUNCTION_Str, 25, OUTPUT_DIGITS_LENGTH,
         STRESS_FUNCTION(x));
  // SIGNBIT ОЧЕН коварен нужно приводить к одному типу!
  int y = abs(stress) - abs(healthy);
  if (abs(y) < 1)  // //ВНИМАНИЕ ТУТ НАСТРАИВАЕТСЯ ТОЧНОСТЬ CРАВНЕНИЯ!
    result = 1;
  return result;
}
...
int main() {
  ...
  if (ntests_failed == 0) {  // делаем красоту
    printf("%s monroebu.passed \033[0;32m\u2713\033[0m\n",
           STRESS_FUNCTION_Str);  // делаем красоту
  } else {                        // делаем красоту
    printf("%s monroebu.failed № Failed %d \033[0;31m\u2717\033[0m\n ",
